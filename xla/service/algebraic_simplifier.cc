...
Status AlgebraicSimplifierVisitor::HandleSlice(HloInstruction* slice) {
  ...
  if (HloInstruction * reduce_window;
    ...
    // reduce + reshape if all dimensions either have a window size of one or the
    // entire dimension. No stride or dilation are expected. reduce_window pad
    // should be present to make output shape equal to input shape.
    ...
    auto effective_reduce_dims = [&] {
      ...
      if (rank != window.dimensions_size() ||
          rank != slice->shape().dimensions_size()) {
        return DimensionVector{};
      }
